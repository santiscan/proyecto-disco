...
buttons = document.querySelectorAll('.ticketButton')
...
numberOfTicketSpans = document.querySelectorAll('.numberOfTickets')

for(let i = 0; i < numberOfTicketSpans.length; i++) {
    let cityName = numberOfTicketSpans[i].id.slice(0, numberOfTicketSpans[i].id.indexOf('NumberOfTickets'));
...
let getTickets = (place, noTickets) => {
    if(noTickets < 1) {
        swal("No tickes available", `Sorry, the concert in ${place} is sold out :(`, "error");
    } else{
        tickets[place]--
        disableSoldOutButtons(place);
        swal("Ticket purchased!", `You have your ticket for the concert in ${place}. Enjoy!`, "success");
    }
}
...
function disableSoldOutButtons(place) {
    document.querySelector(`#${place}NumberOfTickets`).textContent = tickets[place];
        if(tickets[place] < 1) {
